...
char *GetPCIDevProgName(u8 progIF)
{
    int i;
...
void PrintPCIDevHeader(u32 bus, u32 dev, u32 func, PCIDevHeader *p_pciDevice)
{
	...
	printf("Bus:Dev:Func=0x%02x:%02x:%02x vID=0x%04x dID=0x%04x ClCode=0x%02x SbClCode=0x%02x PIF=0x%02x", 			bus, dev, func, p_pciDevice->option.vendorID, p_pciDevice->option.deviceID,
